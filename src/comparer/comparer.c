...
int main(int argc, char **argv) {
  ...
  int f_c_ret = first_check(image_per1, image_per2);
  if (f_c_ret != 0) {
    free_bmp(image_per1);
    free_bmp(image_per2);
    return f_c_ret;
  ...
  int cmp_ret = comparer(image_per1, image_per2);
  if (cmp_ret != 0) {
    free_bmp(image_per1);
    free_bmp(image_per2);
    return cmp_ret;
...
int first_check(BMPImage *image1, BMPImage *image2) {
  ...
  if (image1->infoHeader.width != image2->infoHeader.width) {
    fprintf(stderr, "Error: resolution across the width of the image lines "
                    "does not match.\n");
    return 1;
  }
...
int comparer(BMPImage *image1, BMPImage *image2) {
  if (image1->infoHeader.bitCount == 24 || image1->infoHeader.bitCount == 8) {
    int diff_count = 0;
    size_t width = image1->infoHeader.width;
    size_t height = abs(image1->infoHeader.height);
    ...
    size_t stride = row_size + padding;
